<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="任务主题" prop="renwuzhuti">
          <el-input v-model="ruleForm.renwuzhuti" 
              placeholder="任务主题" clearable  :readonly="ro.renwuzhuti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="任务主题" prop="renwuzhuti">
              <el-input v-model="ruleForm.renwuzhuti" 
                placeholder="任务主题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="任务进度" prop="renwujindu">
          <el-select v-model="ruleForm.renwujindu" placeholder="请选择任务进度">
            <el-option
                v-for="(item,index) in renwujinduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="任务进度" prop="renwujindu">
	      <el-input v-model="ruleForm.renwujindu"
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='renwuzhuti'){
            this.ruleForm.renwuzhuti = obj[o];
	    this.ro.renwuzhuti = true;
            continue;
          }
          if(o=='kaishishijian'){
	...
	this.ruleForm.canyurenyuan = this.ruleForm.canyurenyuan.replace(reg,'../../../ssm7x283/upload');
	this.ruleForm.renwuneirong = this.ruleForm.renwuneirong.replace(reg,'../../../ssm7x283/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gongzuojihua/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongzuojihuaCrossAddOrUpdateFlag = false;
                  this.parent.search();
