<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="出差时间" prop="chuchashijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.chuchashijian" 
                type="date"
                placeholder="出差时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.chuchashijian" label="出差时间" prop="chuchashijian">
              <el-input v-model="ruleForm.chuchashijian" 
                placeholder="出差时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="出差天数" prop="chuchatianshu">
          <el-input v-model="ruleForm.chuchatianshu" 
              placeholder="出差天数" clearable  :readonly="ro.chuchatianshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="出差天数" prop="chuchatianshu">
              <el-input v-model="ruleForm.chuchatianshu" 
                placeholder="出差天数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.qingkuangshuoming" label="情况说明" prop="qingkuangshuoming">
                    <span>{{ruleForm.qingkuangshuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
	...
	shhf : false,
      ...
      rules: {
          ...
          chuchashijian: [
                { required: true, message: '出差时间不能为空', trigger: 'blur' },
          ],
          chuchatianshu: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuangongxingming = true;
          ...
          if(o=='chuchashijian'){
            this.ruleForm.chuchashijian = obj[o];
	    this.ro.chuchashijian = true;
            continue;
          }
          if(o=='chuchatianshu'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `chuchashenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.chuchashenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
